...
the repo) because the individual scripts there all start a client in their
respective setup routine. That is, they all start the Java VM, which will fail
once pyTest gets to the second script in the sequence. Instead, we run pyTest
for each test group separately, with the coverage plug-in enabled, and thus
generate the coverage report incrementally.

We also render the coverage report (in `.coverage`) as static HTML for easy
inspection. This is helpful during development. Find it in the `build/coverage`
folder.
